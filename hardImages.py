...
if number == "default":
    ...
    minNoise = int(input("What is the minimum amout of shapes you want?: "))
    maxNoise = int(input("Also, what is the maximum amout of shapes you want?: "))
...
for num in range(int(number)):
    ...
    noise = random.randint(minNoise, maxNoise)

    
    for i in range(noise):
        shapeSelector = random.randint(0, 2)
        if shapeSelector == 0:
            rect()
            img = cv2.rectangle(img, start_point, end_point, color, thickness)

        elif shapeSelector == 1:
            circ() 
    ...
    shapeSelector = random.randint(0, 1)
    if shapeSelector == 0:
        img = cv2.rectangle(img, (210, 130), (410, 330), (255, 100, 203), -1)
    if shapeSelector == 1:
        img = cv2.circle(img, (310, 230), 100, (255, 100, 203), -1)

    if shapeSelector == 0:
        cv2.imwrite(f'images/{num + 1}_square.jpg', img)
    elif shapeSelector == 1:
        cv2.imwrite(f'images/{num + 1}_circle.jpg', img)
...
# identifier - circle or rectangle